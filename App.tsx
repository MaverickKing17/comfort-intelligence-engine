...
const App: React.FC = () => {
  ...
  const renderDashboard = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiTile label="Active Alerts" value="12" trend="+2 today" trendColor="text-red-400" onClick={() => setCurrentView('queue')} />
        <KpiTile label="Dry Runs Prevented" value="48" trend="85% Success" />
        <KpiTile label="Leads Identified" value="15" trend="$42k Pipe" onClick={() => setCurrentView('opportunities')} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
           {urgentLeads.map(system => (
             <SpotlightCard key={system.id} goldGlow={true} className="cursor-pointer" onClick={() => setSelectedSystem(system)}>
               <div className="p-5 h-full flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-amber-400 fill-amber-400" /><span className="text-amber-400 text-[10px] font-bold uppercase">High-Margin Lead</span></div>
                  </div>
                  <h3 className="text-lg font-semibold text-white">{system.address}</h3>
                  <p className="text-xs text-gray-500 mt-1">{system.ownerName}</p>
                  <div className="mt-6 flex justify-between items-center"><span className="text-xs text-amber-500 font-medium">View Triage Detail →</span><span className="text-[10px] bg-amber-500/10 text-amber-500 px-2 py-1 rounded">Rebate Qualified</span></div>
               </div>
             </SpotlightCard>
           ))}
           {MOCK_SYSTEMS.filter(s => s.insights.length === 0 || !s.insights[0].isUrgent).map(system => (
              <SpotlightCard key={system.id} className="cursor-pointer" onClick={() => setSelectedSystem(system)}>
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
          ...
          <SpotlightCard className="p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm"><span className="text-gray-400">Grid Load</span><span className="text-amber-400">Peak Demand</span></div>
              <div className="flex justify-between items-center text-sm"><span className="text-gray-400">Outside Temp</span><span className="text-blue-400">-12°C Tonight</span></div>
            </div>
          </SpotlightCard>
        </div>
      </div>
  ...
  const renderQueue = () => (
    <div className="animate-in slide-in-from-bottom-4 duration-500">
      <header className="mb-6 flex justify-between items-center">
        <div><h2 className="text-2xl font-bold text-white">Triage Queue</h2><p className="text-gray-400 text-sm">Priority calls pending triage decisions</p></div>
        <div className="flex gap-2"><button className="bg-white/5 border border-border px-4 py-2 rounded-lg text-sm flex items-center gap-2"><ListFilter className="w-4 h-4" /> Filter</button></div>
      </header>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-border text-sm">
            {MOCK_TRIAGE_QUEUE.map(item => (
              <tr key={item.id} className="hover:bg-white/5 transition cursor-pointer group" onClick={() => setSelectedSystem(MOCK_SYSTEMS[0])}>
                ...
                <td className="px-6 py-4 text-right"><button className="text-indigo-400 hover:underline">Open Triage</button></td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  const renderOpportunities = () => (
    <div className="animate-in slide-in-from-bottom-4 duration-500">
      <header className="mb-6"><h2 className="text-2xl font-bold text-white">Lead Pipeline</h2><p className="text-gray-400 text-sm">High-margin replacement and agreement leads</p></header>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {MOCK_OPPORTUNITIES.map(opp => (
          <SpotlightCard key={opp.id} className="p-6">
             <div className="flex justify-between items-start mb-4">
             ...
             <button className="w-full bg-white text-black py-2 rounded font-bold text-xs hover:bg-gray-200 transition">Email Quote</button>
          </SpotlightCard>
        ))}
      </div>
    </div>
  ...
  const renderSettings = () => (
    <div className="max-w-4xl animate-in fade-in duration-500 space-y-8">
      <header className="mb-6"><h2 className="text-2xl font-bold text-white">Settings & Integrations</h2><p className="text-gray-400 text-sm">Configure hardware and software connections</p></header>
          ...
          <div className="space-y-4">
            {['Ecobee', 'Nest', 'Honeywell'].map(v => (
              <div key={v} className="flex justify-between items-center p-3 bg-black/20 rounded-lg border border-border">
                ...
                <button className="text-xs bg-indigo-600 px-3 py-1 rounded font-bold">Connect OAuth</button>
              </div>
            ))}
          </div>
        </SpotlightCard>
        ...
        <SpotlightCard className="p-6">
          ...
          <div className="space-y-4">
            {['ServiceTitan', 'Jobber', 'Housecall Pro'].map(v => (
              <div key={v} className="flex justify-between items-center p-3 bg-black/20 rounded-lg border border-border">
                <span className="text-sm">{v}</span>
                <button className="text-xs bg-emerald-600 px-3 py-1 rounded font-bold">Configure API</button>
              </div>
            ))}
          </div>
        </SpotlightCard>
    ...
    <div className="min-h-screen bg-bg text-gray-100 font-sans selection:bg-white/20">
      <nav className="sticky top-0 z-30 bg-bg/80 backdrop-blur-md border-b border-border h-16 flex items-center justify-between px-6">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3"><div className="w-8 h-8 bg-indigo-600 rounded flex items-center justify-center font-bold">At</div><span className="font-bold tracking-tight">Ambient Twin</span></div>
          <div className="hidden lg:flex items-center gap-4 text-sm font-medium">
             <button onClick={() => setCurrentView('dashboard')} className={`px-3 py-1.5 rounded-lg transition ${currentView === 'dashboard' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}>Dashboard</button>
             <button onClick={() => setCurrentView('queue')} className={`px-3 py-1.5 rounded-lg transition ${currentView === 'queue' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}>Triage Queue</button>
             <button onClick={() => setCurrentView('opportunities')} className={`px-3 py-1.5 rounded-lg transition ${currentView === 'opportunities' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}>Opportunities</button>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => setCurrentView('settings')} className="p-2 hover:bg-white/5 rounded-full transition"><Settings className="w-5 h-5 text-gray-500" /></button>
          <button className="p-2 hover:bg-white/5 rounded-full relative transition"><Bell className="w-5 h-5 text-gray-500" /><span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border-2 border-bg"></span></button>
        </div>
      </nav>
